...
// NOTE: renamed OK -> FP_OK. The ESP32 Arduino core (esp32-hal.h, pulled in
// via Arduino.h) already defines a global symbol called OK, and since this
// is a plain (unscoped) enum, its members leak into the enclosing namespace.
// That caused: "error: 'OK' conflicts with a previous declaration".
enum Status : uint8_t {
  FP_OK              = 0x00,
...
void handleVerifyPassword(const uint8_t *data, uint16_t len) {
  ...
  authenticated = ok;
  sendSimpleStatus(ok ? FP_OK : ERR_PASSFAIL);
}
...
void handleMatch() {
  ...
  if (code1 == code2) {
    const uint8_t payload[] = { FP_OK, 0x00, 0x64 };
    sendAckPacket(payload, sizeof(payload));
  } else {
